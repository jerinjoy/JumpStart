...
class SourceGenerator:
    ...
    def assign_addresses_to_mapping_for_stage(self, mapping_dict, stage):
        ...
        for target_mmu in MemoryMapping.get_supported_targets():
            ...
            temp_address = self.get_next_available_dest_addr_after_last_mapping(
                target_mmu,
                stage,
                mapping_dict["page_size"],
                mapping_dict["pma_memory_type"],
            )
        ...
        for target_mmu in MemoryMapping.get_supported_targets():
            ...
            for stage in TranslationStage.get_enabled_stages():
                ...
                self.memory_map[target_mmu][stage].insert(
                    len(self.memory_map[target_mmu][stage]),
                    per_stage_pagetable_mappings[stage],
                )
    ...
    def add_jumpstart_sections_to_mappings(self):
        ...
        # If UART is enabled and we're using virtual memory, add an identity
        # mapping for the UART MMIO page so S-mode can access it.
        # This is done AFTER the pagetable placement calculation so the
        # fixed-address UART MMIO mapping doesn't affect pagetable location.
        for stage in TranslationStage.get_enabled_stages():
            if (
                self.jumpstart_source_attributes["diag_attributes"].get("enable_uart", False)
                and TranslationStage.get_selected_mode_for_stage(stage) != "bare"
            ):
                uart_base = self.jumpstart_source_attributes["defines"]["UART_BASE_ADDRESS"]
                uart_page_aligned = uart_base & ~0xFFF
                source_addr_type = TranslationStage.get_translates_from(stage)
                dest_addr_type = TranslationStage.get_translates_to(stage)
                uart_mapping = {
                    source_addr_type: uart_page_aligned,
                    dest_addr_type: uart_page_aligned,
                    "xwr": "0b011",
                    "page_size": PageSize.SIZE_4K,
                    "num_pages": 1,
                    "pma_memory_type": "wb",
                    "target_mmu": [target_mmu],
                    "translation_stage": stage,
                }
                self.memory_map[target_mmu][stage].insert(
                    len(self.memory_map[target_mmu][stage]),
                    MemoryMapping(uart_mapping, self.max_num_cpus_supported),
                )
    ...
    def generate_rust_data_structures_file(self, output_rust_data_structures_file):
        ...
        def rust_type_for_define(name, value):
            """Infer the Rust type for a define value.
            Uses usize for address-like defines (name contains 'ADDRESS'),
            u64 for all other integer values."""
            if "ADDRESS" in name.upper():
                return "usize"
            return "u64"

        with open(output_rust_data_structures_file, "w") as file_descriptor:
            file_descriptor.write(
                f"// This file is auto-generated by {sys.argv[0]} from {self.diag_attributes_yaml}\n\n"
            )

            # Generate Rust constants from the 'defines' section
            file_descriptor.write("// Jumpstart defines as Rust constants\n")
            for define_name in self.jumpstart_source_attributes["defines"]:
                define_value = self.jumpstart_source_attributes["defines"][define_name]
                if isinstance(define_value, int):
                    rust_type = rust_type_for_define(define_name, define_value)
                    file_descriptor.write(
                        f"#[allow(dead_code)]\n"
                        f"pub const {define_name}: {rust_type} = 0x{define_value:x};\n"
                    )
            file_descriptor.write("\n")

            for c_struct in self.c_structs:
                # Generate constants for array field counts
                for field in c_struct.fields:
                    if field.num_elements > 1:
                        file_descriptor.write(
                            f"#[allow(dead_code, non_snake_case, non_camel_case_types)]\n"
                            f"pub const NUM_{field.name.upper()}: usize = {field.num_elements};\n"
                        )

                file_descriptor.write(
                    "\n#[allow(dead_code, non_snake_case, non_camel_case_types)]\n"
                )
                file_descriptor.write("#[repr(C)]\n")
                file_descriptor.write(f"pub struct {c_struct.name} {{\n")
                for field in c_struct.fields:
                    rust_type = c_type_to_rust_type(field.field_type)
                    if field.num_elements > 1:
                        file_descriptor.write(
                            f"    pub {field.name}: [{rust_type}; {field.num_elements}],\n"
                        )
                    else:
                        file_descriptor.write(f"    pub {field.name}: {rust_type},\n")
                file_descriptor.write("}\n\n")
...
"""
    ...
    def translate_stage(self, target_mmu, stage, source_address):
        ...
        dest_address += BitField.extract_bits(
            source_address,
            (attributes.get_attribute("va_vpn_bits")[current_level][1] - 1, 0),
        )
...
def main():
    ...
    parser.add_argument(
        "--output_rust_data_structures_file",
        help="Data structures file to generate with Rust struct definitions",
        required=False,
        type=str,
    )
    parser.add_argument(
    ...
    if args.output_rust_data_structures_file is not None:
        source_generator.generate_rust_data_structures_file(args.output_rust_data_structures_file)
